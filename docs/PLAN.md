...
| **2a.2** | **Date authority + folder coherence framework** + clock-drift rule + MEDIUM-confidence y/n prompter. Each file's capture date gets an explicit authority score (`exif > companion > filename > mtime`); files >24 h off the folder median get flagged with their source, user picks shift / keep / delete. `--yes-high` / `--yes-medium` flags. | Fuji-3h-behind fixture proposes shift; Y → exiftool writes; re-audit clean. Outlier fixture prompts with reason. | pending |
| **2a.3** | Remaining tagging rules (auto-suggest on notes scaffold is shipped; still to do: auto-propose MEDIUM tag additions from folder-name/cam/prefix for folders where the notes `tags:` list was hand-edited and may be missing something obvious). | Promoted fixture appears in Immich with correct tag hierarchy; MEDIUM prompts surface missing obvious tags. | partial |
| **2a.4** | `immy promote`: rsync + Immich API library-scan trigger, dry-run flag. **Post-scan stack API call for `.insv` ↔ `.lrv` pairs (+ any stitched proxy later)** — `.lrv` becomes the primary visible asset; `.insv` stacked underneath, still downloadable. | Full `audit → promote` on one real pilot trip lands clean in Immich with tags, GPS, correct dates, and 360 pairs visually collapsed to a single tile per shot. | pending |
| **2a.5** | Remaining LOW rules: interactive `trip-timezone` prompt (the HIGH-declarative path shipped in 2a.1+), export-date trap, bloat-candidate flag (feeds Phase 2c), MakerNote cleanup, `geotag-from-gpx`, place-name → coords geocoding. | Two real trips each go through with <10 % LOW-confidence prompts. | partial (GPS + tz declarative done; rest pending) |
