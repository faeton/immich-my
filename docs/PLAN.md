...
| # | Scope | Done when |
|---|---|---|
| **2a.0** | Skeleton + fixtures. Typer CLI stubs, pyexiftool wrapper, empty rules engine, `tests/fixtures/` with hand-crafted problem trees, pytest green on one trivial test. Per-file audit output (`immy audit --verbose` lists each file with its issue, not just aggregates). | `immy audit ./fixtures/dji-srt-pair` prints an EXIF table + per-file flags, exits 0. |
| **2a.1** | Four HIGH-confidence rules, XMP sidecar writer, `state.yml` + folder notes file persistence, JSONL audit log. Rules: (1) `dji-gps-from-srt`, (2) `dji-date-from-srt` — DJI MP4s carry no `DateTimeOriginal`, it's in the SRT; (3) `date-from-filename-vid-img` — parse `VID_YYYYMMDD_HHMMSS` / `IMG_YYYYMMDD_` when EXIF date missing (Insta360, generic action cams); (4) `insta360-pair-by-ts-serial` — `.insv` ↔ `.lrv` share timestamp+serial, not stem. | Audit on real drone + 360 fixtures writes correct XMP sidecars; re-run is a no-op; folder notes file created with trip identity block. |
| **2a.2** | **Date authority + folder coherence framework** + clock-drift rule + MEDIUM-confidence y/n prompter. Each file's capture date gets an explicit authority score (`exif > companion > filename > mtime`); files >24 h off the folder median get flagged with their source, user picks shift / keep / delete. `--yes-high` / `--yes-medium` flags. | Fuji-3h-behind fixture proposes shift; Y → exiftool writes; re-audit clean. Outlier fixture prompts with reason. |
| **2a.3** | Tagging rules: folder-name → `Events/…`, Make/Model → `Gear/…`, filename prefix/ext → `Source/…`. XMP `HierarchicalSubject` writes. Tags also mirrored into folder notes file front-matter. | Promoted fixture appears in Immich with correct tag hierarchy. |
| **2a.4** | `immy promote`: rsync + Immich API library-scan trigger, dry-run flag. **Post-scan stack API call for `.insv` ↔ `.lrv` pairs (+ any stitched proxy later)** — `.lrv` becomes the primary visible asset; `.insv` stacked underneath, still downloadable. | Full `audit → promote` on one real pilot trip (e.g. `Iceland-Volcano-25`) lands clean in Immich with tags, GPS, correct dates, and 360 pairs visually collapsed to a single tile per shot. |
| **2a.5** | Remaining rules as they surface on real data: **`trip-gps-anchor` (LOW — asks for a place/coords when a whole-trip camera has zero GPS, writes to folder notes file + stamps all assets), `trip-timezone` (LOW — asks once per trip, stored in folder notes file)**, export-date trap, bloat-candidate flag (feeds Phase 2c), MakerNote cleanup, `geotag-from-gpx`. | Two more real trips each go through with <10 % LOW-confidence prompts. |
| **2a.6** | Watcher mode: `launchd` plist, debounced `watchdog` on `~/Documents/Incoming/`, non-interactive `--yes-high`. | Drop a folder in Incoming, walk away; return to either clean promotion or a `NEEDS_REVIEW` file listing open questions. |
| **2a.7** | Web routes for the LOW-confidence cases that don't fit a terminal: `/audit` on the sidecar web app, map picker, thumb grid. | Every rule class has either a terminal or web answer path. |
