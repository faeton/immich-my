...
| **2a.5** | Remaining LOW rules: ✅ interactive `trip-timezone` prompt; ✅ `export-date-trap`; ✅ `trip-timezone-guess-gps` (HIGH `write_notes` — reverse-lookup offline via `timezonefinder` when notes has no `timezone:` but media carry GPS); ✅ `trip-timezone` now respects per-file `EXIF:OffsetTimeOriginal` (iPhones, modern mirrorless). Pending: bloat-candidate flag (feeds Phase 2c), MakerNote cleanup, `geotag-from-gpx`, place-name → coords geocoding. | Two real trips each go through with <10 % LOW-confidence prompts. | partial (tz fully auto-fills from GPS; GPX/geocode/bloat/MakerNote pending) |
