...
| **2a.5** | Remaining LOW rules: ✅ interactive `trip-timezone` prompt; ✅ `export-date-trap`; ✅ `trip-timezone-guess-gps`; ✅ `trip-timezone` respects per-file `EXIF:OffsetTimeOriginal`; ✅ `bloat-candidate`; ✅ `geotag-from-gpx` (HIGH — nearest-time-within-5-min match against any `.gpx` track in the folder; honours per-file offset, date suffix, or notes timezone). Pending: MakerNote cleanup, place-name → coords geocoding. | Two real trips each go through with <10 % LOW-confidence prompts. | partial (tz + bloat + GPX + export traps done; geocode/MakerNote pending) |
