...
| **2a.5** | Remaining LOW rules: ✅ interactive `trip-timezone` prompt; ✅ `export-date-trap`; ✅ `trip-timezone-guess-gps`; ✅ `trip-timezone` respects per-file `EXIF:OffsetTimeOriginal`; ✅ `bloat-candidate` (video bits/pixel/frame scoring + source/preserve allowlist — feeds Phase 2c). Pending: MakerNote cleanup, `geotag-from-gpx`, place-name → coords geocoding. | Two real trips each go through with <10 % LOW-confidence prompts. | partial (tz + bloat + export traps done; GPX/geocode/MakerNote pending) |
