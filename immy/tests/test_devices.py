...
def test_dji_still_module_code_mapped():
    # Model is deliberately bare — no redundant "DJI" prefix, since make is
    # already "DJI" and Immich's UI concatenates make + model (would render
    # "DJI DJI Air 3" otherwise — found live 2026-07-12 already affecting 9
    # assets from the normal ingest path before this fix).
    assert devices.resolve("DJI", "FC8282") == ("DJI", "Air 3")
    assert devices.resolve("DJI", "FC8482") == ("DJI", "Mini 4 Pro")
    assert devices.resolve("DJI", "FC9313") == ("DJI", "Mini 5 Pro")
    assert devices.resolve("DJI", "FC3582") == ("DJI", "Mini 3 Pro")
    assert devices.resolve(None, "L2D-20c") == ("DJI", "Mavic 3")
    assert devices.resolve(None, "AC002") == ("DJI", "Osmo Action 3")
    # FC4170 (Mavic 3 tele) vs L2D-20c (Mavic 3 Hasselblad wide) — same drone.
    assert devices.resolve(None, "FC4170") == ("DJI", "Mavic 3 Tele")


def test_dji_code_lookup_case_insensitive():
    assert devices.resolve(None, "fc8482") == ("DJI", "Mini 4 Pro")


def test_dji_video_encoder_fallback():
    # DJI video: Make/Model empty, real model in the Encoder atom.
    assert devices.resolve(None, None, "DJIMavic3Cine") == ("DJI", "Mavic 3 Cine")
    assert devices.resolve(None, None, "DJI Mini4 Pro") == ("DJI", "Mini 4 Pro")
    # Same drone as FC9313 (stills) — video writes a differently-spaced
    # encoder string. Found live 2026-07-12 on 518 assets.
    assert devices.resolve(None, None, "DJI Mini5Pro") == ("DJI", "Mini 5 Pro")
